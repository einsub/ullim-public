...
export default async function JotDailySupport({
  params,
}: {
  ...
  const sup = dict.jotDaily.support;
    ...
    <div className="flex flex-col min-h-screen px-6 pt-14">
      <article className="mx-auto w-full max-w-lg py-16">
        ...
        <section className="mt-8 space-y-6 text-sm leading-relaxed text-white/70">
          ...
          <div>
            <h2 className="font-semibold text-white/90">{sup.howToUseTitle}</h2>
            <ul className="mt-1 list-disc pl-5 space-y-1">
              {sup.howToUse.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
